...
namespace PacketDefinitions420.PacketDefinitions.S2C
{
    public class BeginAutoAttack : BasePacket
    {
            ...
            : base(PacketCmd.PKT_S2C_BEGIN_AUTO_ATTACK, attacker.NetId)
        {
            ...
            Write((byte)0x01);
            Write(MovementVector.TargetXToNormalFormat(navGrid, attacked.Position.X));
            Write((byte)0x80);
            Write((byte)0x01);
            Write(MovementVector.TargetYToNormalFormat(navGrid, attacked.Position.Y));
            Write((byte)0xCC);
            Write((byte)0x35);
            Write((byte)0xC4);
            Write((byte)0xD1);
            Write((float)attacker.Position.X);
            Write((float)attacker.Position.Y);
        }
    }
}