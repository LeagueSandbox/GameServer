...
namespace LeagueSandbox.GameServerTests.Tests.Items
{
    ...
    public class InventoryManagerTests
    {
        ...
        public void TestAddAndGetItem()
        {
            ...
            Assert.AreEqual(0, inventoryManager.GetItemSlot(item.Item1));

            // Make sure the added item and the one we fetch by it's slot are the same object
            var receivedItem = inventoryManager.GetItem(inventoryManager.GetItemSlot(item.Item1));
            Assert.AreEqual(item.Item1, receivedItem);
        ...
        public void TestItemStacking()
        {
            ...
            Assert.AreEqual(item1.Item1, manager.GetItem(0));
            Assert.AreEqual(item2.Item1, manager.GetItem(1));

            // Check stack sizes
            Assert.AreEqual(1, item1.Item1.StackCount);
            Assert.AreEqual(1, item2.Item1.StackCount);
            ...
            for(var i = 0; i < itemType2.MaxStacks - 1; i++)
            {
                ...
                Assert.AreEqual(item2.Item1, item2Reference.Item1);
                Assert.AreEqual(1 + i + 1, item2.Item1.StackCount);
            }
            ...
            Assert.IsNull(shouldBeNull.Item1);
        }
        ...
        public void TestGetItemSlot()
        {
            ...
            var slot = manager.GetItemSlot(item.Item1);
            ...
            try
            {
                var fail = manager.GetItemSlot(item.Item1);
                Assert.Fail("This should fail");
            }
            catch (Exception e)
            {
        ...
        public void TestRemoveItem()
        {
            ...
            // Add an item and make sure it gets added to the first (0) slot
            var item = manager.AddItem(itemManager.GetItemType(2001));
            Assert.AreEqual(0, manager.GetItemSlot(item.Item1));

            // Remove the item and make sure it doesn't exist anymore in the inventory
            manager.RemoveItem(manager.GetItemSlot(item.Item1));
        ...
        public void TestSwapItems()
        {
            ...
            Assert.AreEqual(0, manager.GetItemSlot(item1.Item1));
            Assert.AreEqual(1, manager.GetItemSlot(item2.Item1));
            Assert.AreEqual(2, manager.GetItemSlot(item3.Item1));

            // Swap 0 and 2 around and make sure their slots have swapped
            manager.SwapItems(0, 2);
            Assert.AreEqual(2, manager.GetItemSlot(item1.Item1));
            Assert.AreEqual(item1.Item1, manager.GetItem(2));
            Assert.AreEqual(0, manager.GetItemSlot(item3.Item1));
            Assert.AreEqual(item3.Item1, manager.GetItem(0));

            // Swap 0 and 1 around and make sure their slots have swapped
            manager.SwapItems(0, 1);
            Assert.AreEqual(manager.GetItemSlot(item3.Item1), 1);
            Assert.AreEqual(item3.Item1, manager.GetItem(1));
            Assert.AreEqual(manager.GetItemSlot(item2.Item1), 0);
            Assert.AreEqual(item2.Item1, manager.GetItem(0));

            // Swap with null and make sure it works
            manager.SwapItems(0, 3);
            Assert.IsNull(manager.GetItem(0));
            Assert.AreEqual(manager.GetItemSlot(item2.Item1), 3);
            Assert.AreEqual(item2.Item1, manager.GetItem(3));
        ...
        public void TestGetAvailableItems()
        {
            ...
            Assert.AreEqual(component1.Item1, available[0]);
            ...
            Assert.AreEqual(component1.Item1, available[0]);
            Assert.AreEqual(component2.Item1, available[1]);

            // Remove the first component and make sure we still have everything correctly
            manager.RemoveItem(manager.GetItemSlot(component1.Item1));
            available = manager.GetAvailableItems(zephyr.Recipe.GetItems());
            Assert.AreEqual(1, available.Count);
            Assert.AreEqual(component2.Item1, available[0]);

            // Remove the other comopnent as well
            manager.RemoveItem(manager.GetItemSlot(component2.Item1));

            // Add an unrelated item and make sure it exists
            var unrelated = manager.AddItem(itemManager.GetItemType(4001));
            Assert.IsNotNull(manager.GetItem(manager.GetItemSlot(unrelated.Item1)));
